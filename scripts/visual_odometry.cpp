...
int main(int argc, char **argv) {
    ...
    while (!SLAM.isShutDown())
    {
        ...
        // Extract and print the current pose
        Sophus::SE3f pose = SLAM.TrackRGBD(im, depth, timestamp, vImuMeas); // Assuming you have a method to get the current pose
        Eigen::Vector3f translation = pose.translation();
        float x = translation.x();
        float y = translation.y();
        float z = translation.z();
        
        std::cout << "Current Position: X: " << -z << " meters, "
                  << "Y: " << -x << " meters, "
                  << "Z: " << y << " meters" << std::endl;
