...
int main(int argc, char **argv) {
    ...
    while (!SLAM.isShutDown()) {
        ...
        // <---- All code that was previously in the mutex scope, but doesn't need to be, should go here  --->
...
Eigen::Matrix3d R_device_to_world = (Eigen::AngleAxisd(angle.z(), Eigen::Vector3d::UnitZ()) *
        ...
        if (!vImuMeas.empty()) {
            const ORB_SLAM3::IMU::Point& lastPoint = vImuMeas.back();

            // Access members directly:
            position.x += lastPoint.a.x() * (lastPoint.t - timestamp); // Access x component of Eigen::Vector3f
            position.y += lastPoint.a.y() * (lastPoint.t - timestamp); // Access y component of Eigen::Vector3f
            position.z += lastPoint.a.z() * (lastPoint.t - timestamp); // Access z component of Eigen::Vector3f

            velocity.x = lastPoint.a.x();  
            velocity.y = lastPoint.a.y();
            velocity.z = lastPoint.a.z(); 

            acceleration.x = lastPoint.w.x();
            acceleration.y = lastPoint.w.y();
            acceleration.z = lastPoint.w.z();
        }

        // Clear the previous IMU measurements to load the new ones
        vImuMeas.clear();

        // Display the current position, velocity, and acceleration
        ...
        #else
            std::chrono::monotonic_clock::time_point time_End_Process = std::chrono::monotonic_clock::now();
        #endif
        double delta_time = std::chrono::duration_cast<std::chrono::duration<double>>(time_End_Process - time_Start_Process).count();
        fps = 1.0 / delta_time;
        cout << "FPS: " << fps << endl;

        frame_count++;
    } // ----> End of the 'while (!SLAM.isShutDown())' loop
